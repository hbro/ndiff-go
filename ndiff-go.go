...
func compareScan(scanA, scanB *NmapRun) *Diff {
  ...
  for addr, hostA := range hostsA {
    ...
    if !exists {
      diff.RemovedHosts = append(diff.RemovedHosts, addr)
    } else {
      changes := compareHosts(addr, hostA, hostB)
      if changes != nil {
  ...
  // Find new hosts
  for addr := range hostsB {
    if _, exists := hostsA[addr]; !exists {
      diff.NewHosts = append(diff.NewHosts, addr)
    }
  }
...
func compareHosts(addr string, hostA, hostB *Host) *HostDiff {
...
func printTextDiff(diff *Diff) {
  ...
  // Print removed hosts
  for _, addr := range diff.RemovedHosts {
    fmt.Printf("-%s: Host is down\n", addr)
  }

  // Print new hosts
  for _, addr := range diff.NewHosts {
